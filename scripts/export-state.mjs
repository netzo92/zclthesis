...
import {SNAPSHOT_ANCHOR,validateNodeContext} from '../richlist-data.mjs';
import {coins,address,varint,serializedOutput,sha256} from './chainstate.mjs';
const args=process.argv.slice(2);
if(args.length<2||args.length>3)throw Error('Usage: node scripts/export-state.mjs SNAPSHOT_DIR OUTPUT_DIR [NODE_CONTEXT_JSON]');
const [snapshotDir,outputDir,contextFile]=args.map(path=>resolve(path));
const context=contextFile?validateNodeContext(JSON.parse(await readFile(contextFile,'utf8'))):null;
const anchor=context||SNAPSHOT_ANCHOR;
...
try {
 ...
 if(context&&(context.totalZatoshis!==total.toString()||context.utxoCount!==utxos))throw Error('Snapshot totals differ from node RPC context');
